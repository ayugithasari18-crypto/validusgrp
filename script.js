...
function translatePage(lang) {
    ...
    // Set placeholder nominal jika bahasa berubah
    if (lang === 'en') {
        topupInput.setAttribute('placeholder', 'Minimum Rp10,000'); // Tetap IDR
    } else {
         topupInput.setAttribute('placeholder', 'Minimal Rp10.000');
    }

    // Refresh data checklist (karena terjemahan diubah)
...
function copyAmount() {
    ...
    if (amountClean === '' || parseInt(amountClean, 10) < MIN_AMOUNT) {
        alert(body.getAttribute('data-lang') === 'en' ? 'Transfer nominal is not valid to copy.' : 'Nominal transfer belum valid untuk disalin.');
        return;
    }
    ...
    navigator.clipboard.writeText(amountClean).then(() => {
        ...
        alert(body.getAttribute('data-lang') === 'en' ? 'Failed to copy. Try manual copy.' : 'Gagal menyalin. Coba salin manual.');
    });
}
...
// --- FUNGSI INTEGRASI API GET ---

async function fetchBankData() {
    // ... (kode loading state tetap sama) ...
    ...
    try {
        ...
        // ... (kode sukses dan error handling tetap sama) ...
        if (!response.ok) {
        ...
        let errorMessage = error.name === 'AbortError' 
            ? (body.getAttribute('data-lang') === 'en' ? "Failed to load data (Timeout 10 seconds). Check your connection." : "Gagal memuat data (Timeout 10 detik). Cek koneksi Anda.") 
            : error.message;
